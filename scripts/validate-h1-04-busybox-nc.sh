...
DB_PATH="${DB_PATH:-${ROOT_DIR}/db/trace-lens-h1-04.db}"
WORK_DIR="${ROOT_DIR}/runtime/h1-04-busybox-nc"
TRACEE_OUTPUT="${WORK_DIR}/tracee.ndjson"
REPORT_DIR="${WORK_DIR}/exports"
TCP_PORT="${TCP_PORT:-19094}"
...
if ! command -v busybox >/dev/null 2>&1; then
  echo "busybox is required for H1-04 validation" >&2
  exit 1
fi

if ! command -v nc >/dev/null 2>&1; then
  echo "nc is required for the local listener" >&2
  exit 1
fi
...
SCENARIO_CMD="busybox nc 127.0.0.1 ${TCP_PORT} < /etc/passwd"
...
if [[ -z "${ROOT_PID}" ]]; then
  echo "failed to locate the root pid for H1-04 scenario" >&2
  exit 1
fi
...
echo "scenario: H1-04 busybox nc"
